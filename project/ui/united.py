...
# 在 frame_input 中添加新的 Text widget (在现有 result_text 后面)
...
def query_results():
    ...
    try:
        ...
        INSERT INTO Students (id, major)
        VALUES (%s, %s)
        ON DUPLICATE KEY UPDATE major = %s;
        """
        cursor.execute(insert_student_query, (netid, major, major))
        ...
        query_plan = """
        SELECT course_id, semester, units FROM Plan WHERE student_id = %s;
        """
        cursor.execute(query_plan, (netid,))
        result = cursor.fetchall()

        # Step : Separate courses into next semester and future semesters
        next_semester_courses = []
        future_semester_courses = []
    ...
    # 运行longterm_select.sql脚本
        ...
        result_text_2.delete(1.0, tk.END)  # 清除现有内容
        plan1,plan2 = f1.funct('%'+major+'%',semester,"%COMPSCI%")
        # 假设你有两个数组 array1 和 array2
        # 显示第一个数组的内容
        result_text_2.insert(tk.END, "Array 1 Contents:\n")
        for item in plan1:  # 你需要定义这个数组
            result_text_2.insert(tk.END, f"{item}\n")
            
        result_text_2.insert(tk.END, "\nArray 2 Contents:\n")
        for item in plan2:  # 你需要定义这个数组
            result_text_2.insert(tk.END, f"{item}\n")
